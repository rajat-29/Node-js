...
http.createServer(function(request, response) 
{
  ...
  if (request.method === "GET") {
    if (pathname === "/") {
      readAndServe('rajat.html', 'text/html', response);
      ...
      readTasks('task1',function(tasks) {
        response.writeHead(200, {'Content-type': 'application/json'});
        response.write(JSON.stringify(tasks));
        response.end();
      });
    } else if (pathname === "/task2") {
      readTasks('task2',function(tasks) {
        response.writeHead(200, {'Content-type': 'application/json'});
        response.write(JSON.stringify(tasks));
        response.end();
      });
    } else if (pathname === "/task3") {
      readTasks('task3',function(tasks) {
        response.writeHead(200, {'Content-type': 'application/json'});
        response.write(JSON.stringify(tasks));
        response.end();
      });
     ...
     if (pathname === "/task1") {
      readJSONBody(request, function(task) {
        createTask('task1', task.text, function() {
      ...
      readJSONBody(request, function(task) {
        createTask('task2', task.text, function() {
          // must wait until task is stored before returning response
          response.end();
        });
      });
      ...
      readJSONBody(request, function(task) {
        createTask('task3', task.text, function() {
          // must wait until task is stored before returning response
          response.end();
        });
      });
